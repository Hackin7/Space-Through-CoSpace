...
///Movement Items////////////////////////////////////////////
bool canFit(Map *currlocation, int x, int y){
    return !currlocation->isWalkablePixel(x,y);
}
bool reachable1(Map *currlocation, int x1, int y1, int x2, int y2){
	double dx = x2-x1;
	double dy = y2-y1;
	int signX = dx>0 ? 1:-1;
	int signY = dy>0 ? 1:-1;
	...
	if (dx==0){
        for (int k=0;k<abs(dy);k++){
			j += signY;
			if (canFit(currlocation,i,j)){return false;}
		}
