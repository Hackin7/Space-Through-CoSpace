    ...
    for x in range(size[0]):
        for y in range(size[1]):
            ...
            for val, colour in colourData:
                if pixels[x,y] == colour and (data[x][y] != border and data[x][y] != wall):
                    data[x][y] = val
            
            if pixels[x,y] == borderColour: #slow
                data[x][y] = 9
                addBorder(size,pixels,data,x,y,borderColour,border,15)
            if pixels[x,y] == wallColour: #Border
                data[x][y] = 5
                addBorder(size,pixels,data,x,y,wallColour,border,15)
        addPerimeter(data, border, 15)
    return data
