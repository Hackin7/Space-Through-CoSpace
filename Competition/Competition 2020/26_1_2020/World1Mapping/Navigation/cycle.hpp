...
public:
    ...
    ZoneNodes *zn;
    AStar *mapping;
    Zoning(Map *ll,ZoneNodes *zz,AStar *mp){currlocation=ll;zn=zz;mapping=mp;}
...
public:
    ...
    void collectObjects(bool red, bool blue, bool black){
        ...
        if (prevZone != zoneToCollect){
            zn->setVisitedZone(mapping->path[mapping->path.size()-1]);
            cout<<endl<<"Set Zone: "<<zn->getZoneNo(mapping->path[mapping->path.size()-1])<<" ";
        }
        cout<<" Zone to Collect:"<<zoneToCollect<<" ";
