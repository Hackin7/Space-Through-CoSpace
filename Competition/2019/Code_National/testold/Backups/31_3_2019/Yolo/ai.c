...
void rotateTo(double angle){
	...
	{
		WheelLeft=3;WheelRight=1; //Anticlockwise Turn
	}
	else {
		WheelLeft=1;WheelRight=3; //Clockwise Turn
	}
	//Duration = 1;
}
...
bool collectionDijkstra(int collectTime, bool dontmoveon, bool moveon, int sizex, int sizey){
    if ((Time - prevTime > collectTime  || LoadedObjects-prevLoaded >=2) && !dontmoveon || moveon){
        prevTime = Time;
...
void Game1()
{
    ...
    //else if (collectionDijkstra(20,false, false, 50, 40)){}
