// Time: 2019-04-05 16:54:40.781906
...
//bool rotated = 0;
bool between(int lower, int middle, int higher){
    return (middle < higher && middle > lower);
}
bool angleRangeCheck(int theta1 ,int range){
    if ((theta1 + range)>360){
        return (Compass < (theta1 + range)%360 || Compass > (theta1 - range)%360);
    }
    return between((theta1 - range)%360,Compass,(theta1 + range)%360);
}
...
/*
	if  (between((int)angle-range,Compass,(int)angle+range)){
            //printf("YES\n");
		WheelLeft=4;WheelRight=4;
		//LED_1=1;
		return;
...
/*    if (angleRangeCheck(theta, 10))
    {faster=5;smaller=5;}// printf("10range");}
    else if (angleRangeCheck(theta, 20))
    {faster=5;smaller=5;}// printf("20range");}
    else if (angleRangeCheck(theta, 67))
    {faster=5;smaller=4;}// printf("45range");}
    else{*/faster=5;smaller=3;//}
...
/*
void rotateToWorse(double angle){
...
// File: Game1NodesANDCoordinates/2019Fuller.c
#define MAX 43
...
#define OBJPOINTS 5
int objectPoints[OBJPOINTS] = {7,22,33,22,0};
int objectPointsSize[OBJPOINTS][2] = {{12,12},{12,12},{12,12},{12,12},{12,12}};
/*
#define REDPOINTS 2
...
void depositDijkstra(){
    if (LoadedObjects >= 6 || LoadedSuperObj > 0){
        if (!dijkstraStarted){
