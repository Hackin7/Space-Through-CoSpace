...
void trigo(int x, int y, int originX, int originY){
	st.calcTheta(originX,originY,x,y);

    double angl = st.theta;//angle(x,y, originX, originY);
	int range = 10;
    double theta = angl;
	double CWAngleDiff;
  double ACWAngleDiff;
  ...
  int faster = 0, smaller = 0;
	printf("Trigo: ");
    if (AngleDiff<=5)
    {faster=5;smaller=5;printf("5range\n");}
    ...
    {
		faster=3;smaller=2;printf("30range\n");
		if (colorCheck(w2Obj.slowarea,20)){faster = 5;smaller=4;}
	}
    else if (AngleDiff<=67)
    {
		faster=3;smaller=1; printf("67range\n");
		if (colorCheck(w2Obj.slowarea,20)){faster = 5;smaller=3;}
	}
    else if (AngleDiff<=67)
	{
		faster=2;smaller=0;printf("SHIT\n");
		if (colorCheck(w2Obj.slowarea,20)){faster = 5;smaller=1;}
	}
	else
	{
		faster=2;smaller=-2;printf("TurnArooooooundYooooooou\n");
		if (colorCheck(w2Obj.slowarea,20)){faster = 5;smaller=-5;}
	}
	/*
    if (colorCheck(w2Obj.slowarea,20)){
		if (faster<5 && smaller<5)
          faster++;smaller++;
	}*/
...
int colorSensorX = PositionX, colorSensorY = PositionY;
void rotateToSuper(int x, int y){rotateTo(x,y);}//trigo(x, y, colorSensorX, colorSensorY);}
void rotateToFAST(int x, int y){st.trigo(x, y, PositionX, PositionY);}
