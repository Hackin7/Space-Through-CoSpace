...
coordinates nodeSelect(){
	...
	if (node.pos[1] < 2){
		output = prevCoordinates;
		cout<<"0";
	}
	prevCoordinates = mapping.path[i];
...
void moveZone(int val){
	if (location.getPixel(PositionX1,PositionY1) != val){
		...
		if (onetime.toRun()){
			//cout<<"val";
			//posX = 0; posY = 0;
			coordinates c = randomNode(PositionX1, PositionY1, val, 0.75);
			posX = c.pos[0];
			posY = c.pos[1];
			cout<<posX<<","<<posY<<endl;
		}
...
bool superObjAStar(){
	...
	// else if (w2Obj.noSuperObj>0 && reached(SuperObjX,SuperObjY,25,25) &&
			// (SuperObjTimer.timeout(20)) && !(SuperObjTimer.timeout(25))){
