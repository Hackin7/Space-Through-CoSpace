...
bool isCollectedReset(){
	if (checkRed){collectedRed = false;checkRed=false;}
	if (checkBlue){collectedBlue = false;checkBlue = false;}
	if (checkBlack){collectedBlack = false;checkBlack=false;}
	if (checkSuper){collectedSuper = false;checkSuper=false;}
}
