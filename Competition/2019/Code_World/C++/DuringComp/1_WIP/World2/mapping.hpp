...
void colourZoneMap(int x,int y, int val, int xsize, int ysize){
	carmap(val,findmax(x-xsize,0),findmin(x+xsize,360),findmax(y-ysize,0),findmin(y+ysize,270));
}
void colormap(int x, int y)
{
	int PositionX=x, PositionY=y;
  // unfixed location of the stuff, only probability, can coexist
  if (colorCheck(w2Obj.objRed,10)){colourZoneMap(x,y,redcol,30,20);}
  else if (colorCheck(w2Obj.objBlue,10)){colourZoneMap(x,y,cyancol, 30, 20);}
  else if (colorCheck(w2Obj.objBlack,10)){colourZoneMap(x,y,blackcol,30,20);}

  // fixed loc but can coexist
  else if (colorCheck(w2Obj.specialZone,10)){location.rectfill(specialbluecol,findmax(PositionX-30,0),findmin(PositionX+30,360),findmax(PositionY-20,0),findmin(PositionY+20,270));}
  else if (colorCheck(w2Obj.slowarea,10)){location.rectfill(graycol,findmax(PositionX-30,0),findmin(PositionX+30,360),findmax(PositionY-20,0),findmin(PositionY+20,270));}

  // only exists by themselves
	else if (colorCheck(w2Obj.depositOrange,w2Obj.depRange)){squarefill(orangecol,PositionX,PositionY,20);}
