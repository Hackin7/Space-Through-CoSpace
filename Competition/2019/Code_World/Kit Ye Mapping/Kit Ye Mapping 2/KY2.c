...
bool posInfoLost(){
    ...
    {
			if ((WheelLeft<2 && WheelLeft>-2) || (WheelRight>-2 && WheelRight<2)|| (WheelLeft==-WheelRight)) // can ignore i guess? since its probably turning
			{
				PositionX=tx;
				PositionY=tx;
				return;
      ...
      if (colorCheck(slowarea, 15)){speed *= 0.1;} // slow area
...
bool boundary(){
    if ((xreached(22,10) && 0<=Compass && Compass <180)||
        (xreached(343,10) && 180<=Compass && Compass <=360)||
        (yreached(22,10) && 90<=Compass && Compass <=270)||
        (yreached(250,10) && (Compass>270 || Compass <90))){
...
void Game0()
{
  ...
  if (PositionX==0 && PositionY==0) // pos info lost
  {
    if (WheelLeft==WheelRight) // striaght
    {
      ...
      if (colorCheck(slowarea, 20)){speed*=0.1;}
